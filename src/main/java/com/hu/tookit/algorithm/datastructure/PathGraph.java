...
public class PathGraph implements Graph {
	...
	private void initTop() {
		if (!init) {
			for (Object key : table.keySet()) {
				Vertex v = table.get(key);
				setTopNum(v.topNum, v.adjacList);
			}
			init = true;
		}
	}

	private void setTopNum(Object topNum, List<Object> array) {
		if (array != null)
	...
	public void weightedByDijkdtra(Vertex start, Vertex end) {
		...
		while (true) {
			...
			v.known = true;
			if (v.adjacList != null) {
				for (int i = 0, len = v.adjacList.size(); i < len; i++) {
					Vertex w = table.get(v.adjacList.get(i));
					if (!w.known) {
						int minCvw = v.cvw[i];
	...
	private List<Edge> toOrderQueue(HashMap<Object, Vertex> table) {
		...
		return sort(list.toArray(array));
	}
